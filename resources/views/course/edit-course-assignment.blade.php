                ...
                <div class="card height-auto">
                    <div class="card-body">
                        ...
                        <form class="new-added-form" method="POST">
                            @csrf
                            <input type="hidden" name="course_assignment_id" value="{{ $courseAssignment->id }}">
                            <div class="row">
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    <label>Select College <span class="required">*</span></label>
                                    <select class="select2" name="college" id="college" required>
                                        @if(!$colleges->isEmpty())
                                            <option value="{{ $courseAssignment->College->id }}">{{ $courseAssignment->College->college_name }}</option>
                                            @foreach($colleges as $college)
                                                @if($courseAssignment->College->id != $college->id)
                                ...
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    ...
                                    <select class="select2" name="course" id="course" required>
                                        <option value="{{ $courseAssignment->Course->id }}">{{ $courseAssignment->Course->course_name }}</option>
                                    </select>
                                    @error('course')
                                      <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    ...
                                    <select class="select2" name="year" id="year" required>
                                        @if(!$years->isEmpty())
                                            <option value="{{ $courseAssignment->AcademicYear->id }}">{{ $courseAssignment->AcademicYear->year }}</option>
                                            @foreach($years as $year)
                                                @if($courseAssignment->AcademicYear->id != $year->id)
                                ...
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    ...
                                    <select class="select2" name="semester" id="semester" required>
                                        <option value="{{ $courseAssignment->Semester->id }}">{{ $courseAssignment->Semester->semester }}</option>
                                    </select>
                                    @error('semester')
                                      <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    ...
                                    <select class="select2" name="lecturer" id="lecturer" required>
                                        @if(!$lecturers->isEmpty())
                                            <option value="{{ $courseAssignment->User->id }}">{{ $courseAssignment->User->email }} ({{ $courseAssignment->User->user_name }})</option>
                                            @foreach($lecturers as $lecturer)
                                                @if($courseAssignment->User->id != $lecturer->id)
                                    ...
                                    @if($courseAssignment->status == 1) {{'checked'}} @endif
                                    > Active
                                   </label>
                                </div>
