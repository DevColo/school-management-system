                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table display data-table text-nowrap" id="course_table">
                                <thead>
                                    <tr>
                                        <th><label class="">Course</label></th>
                                        <th>Academic Year</th>
                                        <th>Semester</th>
                                        <th>Lecturer</th>
                                        <th>Status</th>
...
@section('js')
  <script src="{{ asset('js/sweetalerts.js') }}"></script>
    ...
    <script type="text/javascript">
        ...
        function dropCourse(enrollment_id) {
          var token = $('meta[name="csrf-token"]').attr('content');
          swal(
              {
                title: "Drop?",
                text: "Confirm you want to drop this course",
                type: "warning",
              ...
              function(isConfirm) {
                if (isConfirm) {
                  $.ajax({
                      type: 'POST',
                      url:  "/drop-course",
                      data: {_token:token,enrollment_id: enrollment_id},
                      success: function(data) {
                        var parsedJson = jQuery.parseJSON(data);
                        if (typeof parsedJson.msg != 'undefined' && parsedJson.msg == 'unauthorized') {
                          swal("Cancelled", "You're unauthorized to activate this course", "error");
                        }else if(typeof parsedJson.msg != 'undefined' && parsedJson.msg == 'error'){
                          swal("Cancelled", "The course was not activated, Contact the System Admin", "error");
                        }else if(typeof parsedJson.msg != 'undefined' && parsedJson.msg == 'limit'){
                          swal("Cancelled", "Maximum days limit to drop course has reached, You can't drop this course.", "error");
                        }else if(typeof parsedJson.msg != 'undefined' && parsedJson.msg == 'success'){
                          swal(
                            {
                              title: "Dropped",
                              text: "The course has been drop successfully.",
                              type: "success",
                            },
                            function(isOk) {
                              if (isOk) {
                  ...
                  swal("Cancelled", "The course was not dropped.", "error");
                }
              }
          );
      }
